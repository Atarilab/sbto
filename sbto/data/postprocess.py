...
def remove_obs_from_data(traj_file_path: str,) -> None:
    ...
    """
    file = np.load(traj_file_path)
    data = {k: v for k, v in file.items() if k != "obs"}
    np.savez_compressed(
        traj_file_path,
        **data
    )
    print(f"'obs' data removed from {traj_file_path}")
...
def post_process_for_rl(
    ...
    if dst_model_path:
        ...
        data_dict = split_x_traj(src_model_path, x_traj)
