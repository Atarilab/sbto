...
def kernel_value(xi, cj, kappa, kernel_type):
    ...
    if kernel_type == 0:
        ...
        return np.exp(-scaled_diff_l1 / kappa)
            ...
            mag_product = nx_sq * ny_sq
            
            # Check 1: Overflow to Infinity
            if np.isinf(mag_product):
                print("DEBUG: Overflow in magnitude product!")
                print("nx_sq:", nx_sq, "ny_sq:", ny_sq)
                # Return a fallback or extreme value
                return 0.0 

            denom = np.sqrt(mag_product) + 1e-10
            ratio = dot / denom
            
            # Check 2: NaN Check
            if np.isnan(ratio):
                print("DEBUG: NaN detected in cosine ratio!")
                return 0.0

            cos_dist = 1.0 - ratio
            
            # Ensure distance isn't negative due to precision errors
            cos_dist = max(0.0, cos_dist) 
...
class PCBOC(CBO):
    def __init__(self, D, cfg):
        ...
        self._alpha_censensus = 0.1
        self._rep = cfg.repulsion
        self._cost_scale = []

        self._delta = np.full(D, self.cfg.delta)
        self._dt = np.full(D, self.cfg.dt)
    ...
    def opt_first_dim(self, n_dim: int = -1):
        ...
        self._rep = self.cfg.repulsion
        Nu=29
        self._alpha_censensus = 0.1
        self._cost_scale.append(0.)
        super().opt_first_dim(n_dim)
        # self._centers[:, self.n_dim-Nu:self.n_dim] = self._centers[:, self.n_dim-2*Nu:self.n_dim-Nu]
        # self._consensus[:, self.n_dim-Nu:self.n_dim] = self._consensus[:, self.n_dim-2*Nu:self.n_dim-Nu]
        self.save_center_it("./test/")

    def save_center_it(self, dirpath):
        import os
        path = os.path.join(dirpath, "centers_it.npz")
        np.savez_compressed(path, centers_it=np.asarray(self._centers_it), p_it=np.asarray(self._p_it))

    def update_distrib_param(self, state: SolverState, samples: Array) -> None:
        # state.mean, _ = self.sampler.estimate_params(samples)
        if self.first_it:
            self.logs["s"] = 1.
        state.cov[:self.n_dim, :self.n_dim] = self.logs["s"]
        self.logs["mean_K"] = self.cluster_kernel_dist[0]
    ...
    def update_mean(self, samples: Array, costs: Array) -> Tuple[int, float]:
        if self.first_it:
            ...
            self._centers[:] = self.state.mean +  self.cfg.delta * noise[:self.N_cluster]

        argmin = costs.argmin()
        cmin = costs[argmin]

        self._centers_it.append(self._centers.copy())
        self._p_it.append(self._p.copy())
        # if self.n_dim == self.D and self.mean_entropy < 0.0005:
        #     arr = np.asarray(self._centers_it)
        #     np.savez_compressed("centers_it.npz", centers_it=arr)


        consensus, centers, self._p[:] = \
            compute_consensus_clustered_numba(
                ...
                self._alpha,
                self._kernel_type,
                repulsion_strength=max(self._rep, 0.)
            )

        IT_FREEZE = 20
        alpha_start = 0.1
        incr = (1. - alpha_start) / IT_FREEZE

        alpha = 1.
        self._consensus[:, :self.n_dim] += alpha * (consensus - self._consensus[:, :self.n_dim])
        self._centers[:, :self.n_dim] += alpha * (centers - self._centers[:, :self.n_dim])

        self._alpha += self.cfg.alpha_incr
        self._alpha_censensus += incr
        self._alpha_censensus = min(1., self._alpha_censensus)

        decrease = 0.992
        self._delta[:self.n_dim] *= decrease
        # decrease = 0.999
        # Nu = 29
        # self._dt[:self.n_dim-Nu] *= decrease
